...
## Roles
customer · admin · pos_user · delivery_driver · super_admin. Route protection is in `middleware.ts`.
...
- Get real eSewa merchant credentials (`ESEWA_PRODUCT_CODE`, `ESEWA_SECRET_KEY`, `ESEWA_MODE=live`) before enabling eSewa in production — sandbox works with no keys set.
