  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          ...
          <div key={s.label} className="card p-5">
            ...
            <p className="mt-1 truncate font-display text-xl font-bold text-slate-900">{s.value}</p>
          </div>
        ))}
      </div>

      <section className="card p-5">
        <h2 className="mb-4 font-display font-bold text-slate-900">Daily revenue — last 14 days</h2>
        <div className="flex h-40 items-end gap-1.5">
          {daily.map((d) => (
            <div key={d.label} className="group flex flex-1 flex-col items-center gap-1">
              <div className="w-full rounded-t-md bg-violet-500 transition-colors group-hover:bg-violet-600"
                   style={{ height: `${Math.max((d.sum / max) * 100, 2)}%` }}
                   title={`${d.label}: ${npr(d.sum)}`} />
              <span className="hidden text-[10px] text-slate-400 sm:block">{d.label.split(" ")[0]}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="card p-5">
          <h2 className="mb-3 font-display font-bold text-slate-900">Top products (30d)</h2>
          <table className="w-full text-sm">
            <tbody>
              ...
              {top.length === 0 && <tr><td className="py-6 text-center text-slate-500">No sales yet.</td></tr>}
            </tbody>
          </table>
        </section>
        <section className="card p-5">
          <h2 className="mb-3 font-display font-bold text-slate-900">Revenue by payment method</h2>
          ...
          {byMethod.size === 0 && <p className="py-6 text-center text-sm text-slate-500">No paid orders yet.</p>}
        </section>
      </div>
    </div>
  );
