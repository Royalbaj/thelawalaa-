        ...
        <tbody>
          {rows.map((p: any) => (
            <tr key={p.id} className="border-b border-slate-100 last:border-0">
              ...
              <td className="px-4 py-3 text-slate-500">{format(new Date(p.created_at), "d MMM yyyy")}</td>
              {showActions && (
                <td className="px-4 py-3">
                  {p.role === "admin" && !isSuperAdmin
                    ? <span className="text-xs text-slate-400">Manage in Supabase</span>
                    : <StaffRowActions userId={p.id} isActive={p.is_active} role={p.role} branches={(branches ?? []) as never} />}
                </td>
              )}
            </tr>
          ))}
          {rows.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">No one here.</td></tr>}
        </tbody>
      </table>
    </div>
  );
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        <form action="/super-admin/staff"><input name="q" defaultValue={q} placeholder="Search by name…" className="input !w-64" /></form>
        <section>
          <h2 className="mb-2 font-display font-bold text-slate-900">Staff</h2>
