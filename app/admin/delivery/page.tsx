  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        <section>
          <h2 className="mb-2 font-display font-bold text-brand-brown">Waiting for a driver</h2>
            ...
            {(unassigned ?? []).map((o: any) => (
              <div key={o.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-stone-500">{format(new Date(o.created_at), "h:mm a")} · {npr(Number(o.total))}</p>
                </div>
          ...
          <div className="card divide-y divide-orange-50">
            {(active ?? []).map((o: any) => (
              <div key={o.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
                <div>
                  <p className="font-mono font-bold">{o.order_number}</p>
                  <p className="text-xs text-stone-500">Driver: {o.deliveries?.profiles?.full_name ?? "—"}</p>
                </div>
      ...
      <section>
        <h2 className="mb-2 font-display font-bold text-brand-brown">Drivers</h2>
        <div className="card divide-y divide-orange-50">
          {(drivers ?? []).map((d) => (
            <div key={d.id} className="flex items-center justify-between px-4 py-3">
              <div>
                <p className="font-bold">{d.full_name}</p>
                <p className="text-xs text-stone-500">{d.vehicle_type ?? "—"} {d.vehicle_number ?? ""}</p>
              </div>
              <span className={cn("flex items-center gap-1.5 text-xs font-bold", d.is_online ? "text-brand-green" : "text-stone-400")}>
                <span className={cn("h-2 w-2 rounded-full", d.is_online ? "bg-brand-green" : "bg-stone-300")} />
                {d.is_online ? "Online" : "Offline"}
              </span>
            </div>
          ))}
          {(drivers ?? []).length === 0 && <p className="px-4 py-6 text-center text-sm text-stone-500">No drivers yet — invite some from Staff.</p>}
        </div>
      </section>
    </div>
  );
